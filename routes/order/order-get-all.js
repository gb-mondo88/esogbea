...
const { ORDER_GET_ALL } = require("../../constants/routes-paths");

const orderGetAllRouter = express.Router();

orderGetAllRouter.get(ORDER_GET_ALL, async (req, res) => {
  try {
    ...
    const allOrdersSnapshot = await admin.firestore().collection("order").get();
...
module.exports = orderGetAllRouter;
